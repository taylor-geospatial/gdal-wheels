#!/bin/bash
# -----------------------------------------------------------------------------
# This script is derived from rasterio's wheel-build recipe (ci/config.sh) and,
# in turn, from matthew-brett/multibuild and rasterio/rasterio-wheels. It has
# been customized and extended for gdal-wheels.
#
# rasterio is Copyright (c) 2013-2021, Mapbox, and licensed under the
# BSD-3-Clause license. The full notice and license text are reproduced in
# THIRD_PARTY_NOTICES.md at the repository root, as required by that license.
# -----------------------------------------------------------------------------
PROJ_VERSION=9.7.1
...
# CMake 4.x refuses to configure projects declaring cmake_minimum_required(<3.5).
# Several bundled deps (notably Arrow's ExternalProject snappy/thrift/etc.) still
# do. Setting this in the environment makes CMake accept them, and unlike a -D
# cache arg it propagates to ExternalProject sub-cmake invocations (which inherit
# the environment).
...
# Fetch giflib from the Debian source mirror (Fastly CDN) instead of SourceForge,
# which intermittently 403s CI runners. Byte-identical to the SF tarball (the
# pinned GIFLIB_SHA256 below verifies it).
GIFLIB_URL="https://deb.debian.org/debian/pool/main/g/giflib/giflib_${GIFLIB_VERSION}.orig.tar.gz"
...
# PostgreSQL source — we build only libpq from it (for GDAL's PG/PostGISRaster
# vector+raster drivers).
POSTGRES_URL="https://ftp.postgresql.org/pub/source/v${POSTGRES_VERSION}/postgresql-${POSTGRES_VERSION}.tar.bz2"
POSTGRES_FNAME="postgresql-${POSTGRES_VERSION}"
POSTGRES_SHA256="fcb7ab38e23b264d1902cb25e6adafb4525a6ebcbd015434aeef9eda80f528d8"
fetch_untar ${POSTGRES_URL} ${POSTGRES_FNAME}.tar.bz2 ${POSTGRES_SHA256}

# Apache Arrow C++ (with Parquet) — for GDAL's (Geo)Arrow and (Geo)Parquet drivers.
ARROW_URL="https://archive.apache.org/dist/arrow/arrow-${ARROW_VERSION}/apache-arrow-${ARROW_VERSION}.tar.gz"
ARROW_FNAME="apache-arrow-${ARROW_VERSION}"
ARROW_SHA256="2dc8da5f8796afe213ecc5e5aba85bb82d91520eff3cf315784a52d0fa61d7fc"
fetch_untar ${ARROW_URL} ${ARROW_FNAME}.tar.gz ${ARROW_SHA256}
...
function build_arrow {
	# Apache Arrow C++ with Parquet, for GDAL's Arrow/Parquet vector drivers.
	# BUNDLED deps lets Arrow build its own thrift/snappy/etc. Allocators off to
	# keep the dependency surface small.
	if [ -e arrow-stamp ]; then return; fi
	echo "Running build_arrow"
	local cmake=cmake
	# Arrow 18.1.0 bundles thrift 0.20.0 and downloads it from dlcdn.apache.org,
	# which 404s once a release is no longer "current" (it moves to the archive).
	# Override the URL to the stable archive host. Bump alongside ARROW_VERSION.
	export ARROW_THRIFT_URL="https://archive.apache.org/dist/thrift/0.20.0/thrift-0.20.0.tar.gz"
	(cd ${ARROW_FNAME}/cpp &&
...
function build_libpq {
	# Build only the libpq client library (+ headers + pg_config) from the
	# PostgreSQL source tree; GDAL's PG / PostGISRaster drivers link against it.
	# Building src/interfaces/libpq pulls in its libpgport/libpgcommon prereqs.
	if [ -e libpq-stamp ]; then return; fi
	echo "Running build_libpq"
	(cd ${POSTGRES_FNAME} &&
		./configure --prefix=$BUILD_PREFIX \
			--without-readline \
			--without-zlib \
			--without-icu \
			--without-zstd &&
		make -C src/interfaces/libpq -j4 &&
		make -C src/interfaces/libpq install &&
		make -C src/include install &&
		make -C src/bin/pg_config -j4 &&
...
function build_gdal {
	...
	(cd ${GDAL_FNAME} &&
		...
		$cmake .. \
			...
			-DGDAL_ENABLE_POSTGISRASTER=ON \
			-DPostgreSQL_ROOT=$BUILD_PREFIX \
			-DPostgreSQL_INCLUDE_DIR=$BUILD_PREFIX/include \
			-DPostgreSQL_LIBRARY=$BUILD_PREFIX/lib/libpq.$lib_ext \
			-DGDAL_USE_ARROW=ON \
			-DOGR_ENABLE_DRIVER_ARROW=ON \
			-DGDAL_USE_PARQUET=ON \
			-DOGR_ENABLE_DRIVER_PARQUET=ON \
			-DArrow_DIR=$BUILD_PREFIX/lib/cmake/Arrow \
			-DParquet_DIR=$BUILD_PREFIX/lib/cmake/Parquet \
			-DArrowDataset_DIR=$BUILD_PREFIX/lib/cmake/ArrowDataset \
